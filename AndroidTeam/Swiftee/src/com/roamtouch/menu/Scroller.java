...
///        float ppi = context.getResources().getDisplayMetrics().density * 160.0f;
    ...
    public float getCurrVelocity() {
    	...
    	//return (float) (Math.PI / (timePassed() / 1000.0f));
...
//        Log.d("Timepassed,duration","timeP:"+timePassed+",duration"+mDuration);
        ...
        if (timePassed < mDuration) {
            ...
            case FLING_MODE:
                ...
                float distance = (1000*mVelocity * timePassedSeconds)
                        - (1000*mDeceleration * timePassedSeconds * timePassedSeconds / 2.0f);
                if(prevAngle == -999){
                	angleChange = distance;
                	prevAngle = angleChange;
                }
                else{
                	angleChange = distance - prevAngle;
                	prevAngle = angleChange;
                }
                //angleChange = distance;
...
//        Log.d("Total distance: ", " "+totalDistance);
        ...
        mFinalX = (int) (startX + Math.round(totalDistance * mCoeffX));
        // Pin to mMinX <= mFinalX <= mMaxX
        mFinalX = Math.min(mFinalX, mMaxX);
        mFinalX = Math.max(mFinalX, mMinX);
        
        mFinalY = (int) (startY + Math.round(totalDistance * mCoeffY));
